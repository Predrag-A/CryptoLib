...
namespace CryptoLib
{
    public class MD5 : ICrypto
    {
        ...
        public byte[] Crypt(byte[] input)
        {
            ...
            // Total number of blocks expected after appending "1" bit; Shifting by 6 because 2^6 = 64
            var blockNum = ((messageLenBytes + 8) >> 6) + 1;

            // Actual message length for encryption after adding padding bits
            var actualLen = blockNum << 6;

            // Padding byte array
            var paddingArray = new byte[actualLen - messageLenBytes];

            // Adding 1000 0000 after message
            paddingArray[0] = 0x80;

            // Adding bit length of unpadded message as 64 bit little-endian integer
            for (var i = 0; i < 8; i++)
            {
            ...
            for (var i = 0; i < blockNum; i++)
            {
                ...
                // Adding result of a segment to the entire result
                a0 += a;
                b0 += b;
                c0 += c;
                d0 += d;
            ...
            // Creating output hash value from final results, each result is 32 bits of the final result
            var md5 = new byte[16];
            var count = 0;
